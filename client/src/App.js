      ...
      <Row>
        <Col md={6}>
          <Form onSubmit={submitHandler}>
            <Form.Group className="mb-3" controlId="source">
              <Form.Label><b>From (Source City):</b></Form.Label>
              <Form.Select className="text-center" onChange={(event)=>{setSource(event.target.value)}}>
                <option>Select Source City</option>
                {
                    cities.map((city)=>(
                      <option value={city} key={city} disabled={city===destination}>
                        {city}
                      </option>
                    ))
                }
            ...
            <Form.Group className="mb-3" controlId="destination">
              <Form.Label><b>To (Destination City):</b></Form.Label>
              <Form.Select className="text-center" onChange={(event)=>{setDestination(event.target.value)}}>
        ...
        <Col md={6}>
          ...
          <Accordion style={{
            ...
            {data.length!==0 && data.flights.map((flight)=>(
              <Accordion.Item eventKey={flight._id} style={{marginBottom:"5px"}}>
                <Accordion.Header>
